...
Inherits TestGroup
	...
	#tag Method, Flags = &h0
		Sub EquivalentToTest()
		  ...
		  // Single key case different from `bd1`. Therefore different.
		  ...
		  // Same as `bd1` but case insensitive.
		  Var bd6 As New BetterDictionary( _
		  False, _
		  "abc" : 1, _
		  "DEF" : True, _
		  "d1" : d1)
		  ...
		  Assert.IsFalse(bd1.EquivalentTo(bd4))
		  Assert.IsFalse(bd1.EquivalentTo(bd3))
		  Assert.IsFalse(bd1.EquivalentTo(bd5))
		  Assert.IsFalse(bd1.EquivalentTo(bd6))
