#tag Class
Protected Class BetterDictionaryCaseInsensitiveTests
Inherits TestGroup
	#tag Event
		Sub Setup()
	...
	#tag Method, Flags = &h21
		Private Function ConstructTestDictionary() As BetterDictionary
		  Var bd As New BetterDictionary(False)
	...
	#tag Method, Flags = &h0
		Sub LookupTest()
		  ...
		  Assert.IsTrue(TestDictionary.Lookup("MIXEDCASE TEXT", 2) = "MixedCase Text:value")
		  Assert.IsTrue(TestDictionary.Lookup("uppercase text", 3) = "UPPERCASE TEXT:value")
		  Assert.IsTrue(TestDictionary.Lookup("UPPERCASE STRING", 4) = "UPPERCASE STRING:value")
		  Assert.IsTrue(TestDictionary.Lookup("MIXEDCASE STRING", 5) = "MixedCase String:value")
		  Assert.IsTrue(TestDictionary.Lookup("lowercase string", 6) = "lowercase string:value")
